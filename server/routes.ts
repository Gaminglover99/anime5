...
function isAdmin(req: Request, res: Response, next: Function) {
  if (req.isAuthenticated()) {
    console.log("User role check:", req.user, (req.user as any).role);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/animes/search", async (req, res) => {
    try {
      ...
      // Filter animes by title only (ignore description) and deduplicate
      const seenIds = new Set();
      const filteredAnimes = allAnimes.filter(anime => {
        const title = anime.title.toLowerCase();
        // Deduplicate results - only include each anime once
  ...
  app.get("/api/animes", async (req, res) => {
    try {
      ...
      if (filters.search && filters.search.trim() !== '') {
        ...
        // Also deduplicate results by ID to avoid showing the same anime multiple times
        const seenIds = new Set();
        filteredAnimes = filteredAnimes.filter(anime => {
          const titleLower = anime.title.toLowerCase();
          // Check if we've already included this anime ID
          if (seenIds.has(anime.id)) {
            return false;
          }
          // If the title matches, add the ID to our seen set and include it
  ...
  app.post("/api/episodes", isAdmin, async (req, res) => {
    try {
      console.log("Episode creation request body:", JSON.stringify(req.body));
      
      // Add duration field if missing
      ...
      if (!validationResult.success) {
        console.error("Episode validation failed:", validationResult.error.format());
        return res.status(400).json({ error: validationResult.error.format() });
      }
  ...
  app.post("/api/video-sources", isAdmin, async (req, res) => {
    try {
      console.log("Video source creation request body:", JSON.stringify(req.body));
      
      // Process YouTube URL if provided
      if (req.body.url && req.body.url.includes('youtube.com')) {
        // Extract video ID from youtube URL
        const videoIdMatch = req.body.url.match(/(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
        
        if (videoIdMatch && videoIdMatch[1]) {
          const videoId = videoIdMatch[1];
          // Replace with embedded URL format
          req.body.url = `https://www.youtube.com/embed/${videoId}`;
          console.log("Converted YouTube URL:", req.body.url);
        }
      }
      
      const validationResult = insertVideoSourceSchema.safeParse(req.body);
      if (!validationResult.success) {
        console.error("Video source validation failed:", validationResult.error.format());
        return res.status(400).json({ error: validationResult.error.format() });
      }
      ...
      console.log("Video source created successfully:", source);
  ...
  app.put("/api/video-sources/:id", isAdmin, async (req, res) => {
    try {
      console.log("Video source update request body:", JSON.stringify(req.body));
      ...
      if (req.body.url && req.body.url.includes('youtube.com')) {
        ...
        if (videoIdMatch && videoIdMatch[1]) {
          ...
          // Replace with embedded URL format
          req.body.url = `https://www.youtube.com/embed/${videoId}`;
          console.log("Converted YouTube URL for update:", req.body.url);
        }
      }
      ...
      console.log("Video source updated successfully:", source);
      res.json(source);
    } catch (error) {
  ...
  // User data sync route - fetches all user preferences in a single request
  app.get("/api/user-data-sync", isAuthenticated, async (req, res) => {
    try {
      const userId = (req.user as any).id;
      
      // Fetch user's watchlist, favorites, and any in-progress episodes
      const [watchlist, favorites] = await Promise.all([
        storage.getWatchlist(userId),
        storage.getFavorites(userId)
      ]);
      
      // Return all user data in a single response
      res.json({
        watchlist,
        favorites,
  ...
  app.get("/api/anime-requests", isAdmin, async (req, res) => {
    try {
      const requests = await storage.getAnimeRequests();
      res.json({ data: requests });
    } catch (error) {
      console.error("Error fetching anime requests:", error);
      res.status(500).json({ message: "Failed to fetch anime requests" });
    }
  });

  app.get("/api/anime-requests/:id", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const request = await storage.getAnimeRequest(id);
      if (!request) {
        return res.status(404).json({ message: "Anime request not found" });
      }
      res.json(request);
    } catch (error) {
      console.error("Error fetching anime request:", error);
      res.status(500).json({ message: "Failed to fetch anime request" });
    }
  });

  app.post("/api/anime-requests", isAuthenticated, async (req, res) => {
    try {
      console.log("Received anime request:", JSON.stringify(req.body));
      
      // Manual validation for fields to provide better error messages
      if (!req.body.animeName) {
        return res.status(400).json({ message: "Anime name is required" });
      }
      
      // Get the schema fields for proper validation
      const validationResult = insertAnimeRequestSchema.safeParse(req.body);
      if (!validationResult.success) {
        console.error("Validation errors:", validationResult.error.format());
        return res.status(400).json({ 
          message: "Invalid anime request data", 
      ...
      const requestData = {
        ...
        userId,
        status: "pending", // Default status for new requests - use lowercase to match DB
      };
      
      console.log("Creating anime request with data:", JSON.stringify(requestData));
      
      const request = await storage.createAnimeRequest(requestData);
      console.log("Anime request created successfully:", request);
      res.status(201).json(request);
    } catch (error: any) {
      console.error("Error creating anime request:", error);
      res.status(500).json({ 
  ...
  app.put("/api/anime-requests/:id/status", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      const request = await storage.updateAnimeRequestStatus(id, lowercaseStatus);
      if (!request) {
        return res.status(404).json({ message: "Anime request not found" });
      }
      ...
      res.status(500).json({ message: "Failed to update anime request status" });
    }
  });

  app.delete("/api/anime-requests/:id", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const success = await storage.deleteAnimeRequest(id);
      if (!success) {
        return res.status(404).json({ message: "Anime request not found" });
      }
      res.status(204).end();
      ...
      res.status(500).json({ message: "Failed to delete anime request" });
    }
  });
