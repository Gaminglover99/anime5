...
export interface IStorage {
  ...
  // Anime Request methods
  getAnimeRequests(): Promise<AnimeRequest[]>;
  getAnimeRequest(id: number): Promise<AnimeRequest | undefined>;
  createAnimeRequest(request: InsertAnimeRequest): Promise<AnimeRequest>;
  updateAnimeRequestStatus(id: number, status: string): Promise<AnimeRequest | undefined>;
  deleteAnimeRequest(id: number): Promise<boolean>;
...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    ...
    const createSessionTable = async () => {
      try {
        ...
        if (!tableExists) {
          ...
          // Table exists, just make sure the index exists
          await pool.query(`
            CREATE INDEX IF NOT EXISTS "IDX_session_expire" 
              ON "session" ("expire");
          `);
    ...
    console.log("PostgreSQL session store initialized with connection to Neon database");
  }
  ...
  async getUserByUsername(username: string): Promise<User | undefined> {
    ...
    const resultByUsername = await db.select().from(users).where(eq(users.username, username));
    if (resultByUsername.length > 0) {
      return resultByUsername[0];
    }
    
    // If not found by username, try with email
    const resultByEmail = await db.select().from(users).where(eq(users.email, username));
  ...
  async deleteSeason(id: number): Promise<boolean> {
    try {
      // First, get all episodes of this season
      const seasonEpisodes = await db.select().from(episodes).where(eq(episodes.seasonId, id));
      
      // Delete each episode (which will cascade to delete video sources)
  ...
  async deleteEpisode(id: number): Promise<boolean> {
    try {
      ...
      const episodeExists = await db.select({ id: episodes.id }).from(episodes).where(eq(episodes.id, id));
      if (episodeExists.length === 0) {
        throw new Error("Episode not found");
      }
      
      // Delete related video sources first
      await db.delete(videoSources).where(eq(videoSources.episodeId, id));
  ...
  // Anime Request methods
  async getAnimeRequests(): Promise<AnimeRequest[]> {
    return await db
      .select()
      .from(animeRequests)
      .orderBy(desc(animeRequests.createdAt));
  }

  async getAnimeRequest(id: number): Promise<AnimeRequest | undefined> {
    const result = await db
      .select()
      .from(animeRequests)
      .where(eq(animeRequests.id, id));
    return result[0];
  }

  async createAnimeRequest(request: InsertAnimeRequest): Promise<AnimeRequest> {
    const [result] = await db
      .insert(animeRequests)
      .values(request)
      .returning();
    return result;
  }

  async updateAnimeRequestStatus(id: number, status: string): Promise<AnimeRequest | undefined> {
    const [result] = await db
      .update(animeRequests)
      .set({ 
        status,
        updatedAt: new Date()
      })
  ...
  async deleteAnimeRequest(id: number): Promise<boolean> {
    const result = await db
      .delete(animeRequests)
      .where(eq(animeRequests.id, id))
      .returning({ id: animeRequests.id });
    return result.length > 0;
  }
}
