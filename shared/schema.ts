...
// Table for anime requests from users
export const animeRequests = pgTable("anime_requests", {
  id: serial("id").primaryKey(),
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  animeRequests: many(animeRequests),
}));
...
export const animeRequestsRelations = relations(animeRequests, ({ one }) => ({
  user: one(users, {
    fields: [animeRequests.userId],
    references: [users.id],
  }),
}));
...
export const insertAnimeRequestSchema = createInsertSchema(animeRequests)
  .omit({ 
    id: true, 
    createdAt: true, 
    updatedAt: true, 
...
export type AnimeRequest = typeof animeRequests.$inferSelect;
export type InsertAnimeRequest = z.infer<typeof insertAnimeRequestSchema>;
